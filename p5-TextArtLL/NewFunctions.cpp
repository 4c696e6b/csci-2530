...
void menuTwo(Node*& current, List& undoList, List& redoList, List& clips, bool& animate)
{
	...
	while (exitFlag)
	{
		...
		case 'F':
		{
			...
			clearLine(MAXROWS + 2, sizeof(menu) + 20);
			...
			fillRecursive(current->item, selPoint1.row, selPoint1.col, current->item[selPoint1.row][selPoint1.col], pointChar1, animate);
			break;
		}
		...
		case 'L':
		{
			...
			drawLine(current->item, selPoint1, selPoint2, animate);
			break;
		}
		...
		case 'B':
		{
			...
			while (cin.fail()) {
				...
				clearLine(MAXROWS + 1, sizeof(menu) + 20);
				cout << "error: not an integer, Enter size: ";
				cin >> height;
			}
			...
			addUndoState(undoList, redoList, current);
			drawBox(current->item, selPoint1, height, animate);
			break;
		}
		...
		case 'N':
		{
			...
			while (cin.fail()) {
				...
				clearLine(MAXROWS + 1, sizeof(menu) + 20);
				cout << "error: not an integer, Enter size of largest box: ";
				cin >> height;
			}
			...
			addUndoState(undoList, redoList, current);
			drawBoxesRecursive(current->item, selPoint1, height, animate);
			break;
		}
		...
		case 'T':
		{
			...
			while (cin.fail()) {
				...
				clearLine(MAXROWS + 1, sizeof(menu) + 20);
				cout << "error: not an integer, Enter approximate tree height: ";
				cin >> height;
			}
			...
			while (cin.fail()) {
				...
				clearLine(MAXROWS + 1, sizeof(menu) + 20);
				cout << "error: not an integer, Enter branch angle: ";
				cin >> branchAngle;
			}
			...
			addUndoState(undoList, redoList, current);
			treeRecursive(current->item, selPoint1, height, 270, branchAngle, animate);
			break;
		}
		...
		clearLine(MAXROWS + 1, sizeof(menu) + 40);
		clearLine(MAXROWS + 2, sizeof(menu) + 40);
	}
}
